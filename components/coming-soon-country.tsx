...
function AnimatedStreaks() {
    ...
    useEffect(() => {
        const newStreaks = Array.from({ length: 20 }).map((_, i) => {
            const isRed = i % 2 === 0;
            return {
                id: i,
                isRed,
                color: isRed ? "bg-red-500" : "bg-cyan-100",
    ...
    return (
        <>
            {streaks.map((streak) => (
                <motion.div
                    key={streak.id}
                    className={`absolute ${streak.color} rounded-full blur-[1px]`}
                    style={{
                        height: `${streak.width}px`,
                        width: `${streak.height}px`,
                        top: `${streak.top}%`,
                        left: "-20%",
                        opacity: streak.opacity,
                        boxShadow: streak.isRed
